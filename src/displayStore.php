...
//validate item
$get_item_sql = "SELECT c.id as category_id, c.cat_title, si.item_title, si.item_price, si.item_desc, si.item_image 
                    FROM store_items
                    AS si LEFT JOIN store_categories
                    AS c on c.id = si.cat_id
...
if (mysqli_num_rows($get_item_res) < 1) {
    ...
    while ($item_info = mysqli_fetch_array($get_item_res)) {
        ...
        $category_title = strtoupper(stripslashes($item_info['cat_title']));
        $item_title = stripslashes($item_info['item_title']);
        $item_price = $item_info['item_price'];
        $item_description = stripslashes($item_info['item_desc']);
